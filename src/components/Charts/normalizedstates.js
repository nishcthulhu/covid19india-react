...
function NormalizedStates(props) {
	...
	const filterData = props.data.filter((object) => {
	...
	const temp = accumulateData(smoothened(filterData,1).filter((obj) => {return obj !== undefined}));
	    ...
	    keys.forEach ((key,index)=>{
	    	if (key === 'date' || key === 'status') {
				...
				.attr("d", d3.line()
			        ...
			        .y(function(d) { return yScale(parseInt(d[key]))
			        }))
